...
const DynamicHomeTabPagesCompo = ({ category, name, title, open, submenu, url, ParamList, RouteName }: P) => {
    ...
    pLOG("homeTopTabCategoryNews:", [homeTopTabCategoryNews]);
    ...
    return (<View>
        <ContentListCoverWithTitle style={{ marginHorizontal: bSpace / 2 }} >
            <ButtonX text={"Scroll to Top" + check} onPress={() => {
                getCategoryListFNN({ _isTopLoading: true });
            }} >
            </ButtonX>
            <FlashList
                data={homeTopTabCategoryNews}
